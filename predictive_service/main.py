    ...
    for p in projects:
        ...
        if expenditure_progress_pct > physical_progress + 15:
            alerts_to_insert.append({
                ...
                "severity": "Warning",
                "status": "Open",
                "recommended_intervention": "Mandatory Field Inspection & Joint Review"
            })
        ...
        if cost_overrun_pct > 15:
            alerts_to_insert.append({
                ...
                "severity": "Critical",
                "status": "Open",
                "recommended_intervention": "Detailed Financial Audit and Re-budgeting Approval"
            })
        ...
        if physical_progress < 5 and time_elapsed_pct > 20:
            alerts_to_insert.append({
                ...
                "severity": "Critical",
                "status": "Open",
                "recommended_intervention": "Issue Show-Cause Notice to Agency"
            })
...
# Attempt to load ML models at startup
try:
    xgb_cost_pipeline = joblib.load(os.path.join(os.path.dirname(__file__), "models", "xgb_cost_pipeline.pkl"))
    xgb_time_pipeline = joblib.load(os.path.join(os.path.dirname(__file__), "models", "xgb_time_pipeline.pkl"))
    
    # Extract regressors and preprocessor for SHAP
    preprocessor = xgb_cost_pipeline.named_steps['preprocessor']
    regressor_cost = xgb_cost_pipeline.named_steps['regressor']
    regressor_time = xgb_time_pipeline.named_steps['regressor']
    
    explainer_cost = shap.TreeExplainer(regressor_cost)
    explainer_time = shap.TreeExplainer(regressor_time)
    
    ML_MODELS_LOADED = True
    print("Successfully loaded XGBoost risk models and SHAP explainers.")
except Exception as e:
    ML_MODELS_LOADED = False
    print("Warning: ML models not found. Train models first to enable /api/v1/predict-risk.", e)
...
class ProjectRiskInput(BaseModel):
    ...
    state: str
    burn_rate_6m: float
    phys_burn_rate_6m: float
    land_acquisition_issue: int
    forest_clearance_issue: int
    contractor_delay: int

@app.post("/api/v1/predict-risk")
def predict_risk_ml(req: ProjectRiskInput):
    if not ML_MODELS_LOADED:
        return {"error": "ML Models not loaded. Please train models first."}
        
    # Convert input to DataFrame
    df = pd.DataFrame([req.model_dump()])
    
    # Make predictions using pipelines
    pred_cost = xgb_cost_pipeline.predict(df)[0]
    pred_time = xgb_time_pipeline.predict(df)[0]
    
    # SHAP Explainability
    # Transform input manually to get SHAP values
    ...
    shap_vals_cost = explainer_cost.shap_values(X_transformed)[0]
    shap_vals_time = explainer_time.shap_values(X_transformed)[0]
    
    # Pair with names
    cost_drivers = sorted(zip(feature_names, shap_vals_cost), key=lambda x: abs(x[1]), reverse=True)
    time_drivers = sorted(zip(feature_names, shap_vals_time), key=lambda x: abs(x[1]), reverse=True)
    
    # Format drivers (Top 3)
    def format_driver(driver):
        name = driver[0].split("__")[-1]
        val = driver[1]
        sign = "+" if val > 0 else ""
        return f"{name}: {sign}{val:.2f}"
        
    top_cost_drivers = {driver[0].split("__")[-1]: float(driver[1]) for driver in cost_drivers[:3]}
    top_time_drivers = {driver[0].split("__")[-1]: float(driver[1]) for driver in time_drivers[:3]}
    
    return {
        "project_id": req.project_id,
        "predictions": {
            "predicted_cost_overrun_pct": round(float(pred_cost), 2),
            "predicted_time_overrun_months": round(float(pred_time), 2)
        },
        "shap_attribution": {
            "top_cost_drivers": top_cost_drivers,
            "top_time_drivers": top_time_drivers
        }
    }
...
@tool
def execute_read_only_sql(query: str):
    '''Executes a read-only SQL query on the Postgres database. Use this to answer complex data questions like filtering projects by states, cost, etc.'''
    try:
        query_upper = query.upper()
        if any(keyword in query_upper for keyword in ["INSERT", "UPDATE", "DELETE", "DROP", "ALTER", "CREATE", "TRUNCATE"]):
             return "Error: Only SELECT queries are permitted."
        
        # Remove trailing semicolon for RPC compatibility
        query = query.strip()
        if query.endswith(';'):
            query = query[:-1]
        ...
        return {"columns": columns, "data": data[:50]}
    except Exception as e:
...
def chat_endpoint(req: ChatRequest):
    try:
        ...
        tools = [get_high_risk_projects, get_project_details, execute_read_only_sql]
        
        system_prompt = """You are a Lead Agentic AI Architect and Project Intelligence Assistant for MoSPI officials.
You have access to read-only SQL query tools. When users ask data-specific questions (e.g. "Which railway projects in Assam have cost overruns > 20%?"), use the execute_read_only_sql tool to fetch verified data from the database.
...
@app.post("/api/v1/predict-risk-bulk")
def predict_risk_ml_bulk(reqs: List[ProjectRiskInput]):
    if not ML_MODELS_LOADED:
        return {"error": "ML Models not loaded. Please train models first."}
    ...
    df = pd.DataFrame([req.model_dump() for req in reqs])
    
    # Make predictions using pipelines
    pred_costs = xgb_cost_pipeline.predict(df)
    pred_times = xgb_time_pipeline.predict(df)
    
    # SHAP Explainability
    X_transformed = preprocessor.transform(df)
    feature_names = preprocessor.get_feature_names_out()
    
    shap_vals_cost = explainer_cost.shap_values(X_transformed)
    
    results = []
    for i in range(len(reqs)):
        # Calculate cost drivers for each project
        cost_drivers = sorted(zip(feature_names, shap_vals_cost[i]), key=lambda x: abs(x[1]), reverse=True)
        top_cost_drivers = {driver[0].split("__")[-1]: float(driver[1]) for driver in cost_drivers[:3]}
        
        results.append({
            "project_id": reqs[i].project_id,
            "predictions": {
                "predicted_cost_overrun_pct": float(pred_costs[i]),
                "predicted_time_overrun_months": float(pred_times[i])
            },
            "shap_attribution": {
                "top_cost_drivers": top_cost_drivers
            }
