...
async function run() {
    ...
    console.log("Mapped", codeToId.size, "existing projects.");
    ...
    for (const item of jsonData) {
        ...
        if (id) {
            payload.push({
                ...
                pmgid: item.pmgid !== "-" ? item.pmgid : null,
                date_of_approval: item.date_of_approval_mm_yyyy,
                start_date: item.start_date_mm_yyyy,
                original_target_doc: item.original_target_doc_mm_yyyy,
                revised_doc: item.revised_doc_mm_yyyy !== "-" ? item.revised_doc_mm_yyyy : null
            });
        }
    }
    ...
    console.log("Successfully updated all projects to match data.json!");
}
run();
